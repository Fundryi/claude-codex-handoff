...
A question for the human (the Needs decision callout in a Codex reply, and the Result card's question) uses the You pink, because it is addressed to you. In a handoff the reply goes to Claude, so the tag reads "asks you (via Claude)" and the callout "Question for you (via Claude)". In a child agent's session the question is for the lead agent ("asks the lead agent", light teal).
...
- The feed shows everything; there is no filter or "Show internals" switch. Its grammar:
  ...
  - A handoff prompt's `<return_format>` footer is split off into a collapsed Plugin part at the bottom of the card. Tag-only lines in a Claude prompt (never in yours) (`<goal>`, `<rules>`, `<done_when>`) show as small section headings. The answer prefixes are shown as the tag, not repeated in the text.
  - A Codex reply with a real question under Needs decision gets an "asks you" tag, and the whole Needs decision section (never cut short, sub-headed options included, up to the next of the four return-format headings) moves out of the text into a pink "Question for you" callout at the end of the card.
  - The exact text a Resume sends with no prompt typed (the viewer's and the companion's default) gets a neutral gray "resume" tag instead of "handoff".
...
- The feed follows new events automatically while the reader is already near the bottom, and pauses the moment they scroll up. A paused feed shows Jump to latest instead of a toggle, in its own row under the feed, so it never covers a card.
