...
fn main() {
    ...
    let _single_instance = match TcpListener::bind(("127.0.0.1", tray_port)) {
        ...
        Err(_) => {
            if probe_viewer(viewer_port) == ViewerProbe::Viewer {
                ...
                show_error(&format!(
                    "Tray lock port {tray_port} is already in use, but no Codex Live Viewer was found."
                ));
            }
            return;
        }
    ...
    let menu = Menu::new();
    if let Err(error) =
        menu.append_items(&[&open_item, &PredefinedMenuItem::separator(), &exit_item])
    {
    ...
    for y in 0..SIZE {
        for x in 0..SIZE {
            ...
            if visible {
                ...
                rgba[offset..offset + 4].copy_from_slice(&[88, 166, 255, 255]);
            }
        }
    }
