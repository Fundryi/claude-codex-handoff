...
1. Answer by yourself only when you are sure beyond reasonable doubt: the answer is plain from this conversation, the repository, or the spec. Then resume the same thread with `/codex:rescue --resume-thread <thread-id>` and the answer as the prompt, starting with `Answer from Claude (automatic N of 2):`.
