...
test("a terminal job returns immediately and does not time out", async () => {
  ...
  const snapshot = await followJob(process.cwd(), record.id, { budgetMs: 5000 });
...
test("an active job hands back when the budget expires", async () => {
  ...
  const snapshot = await followJob(process.cwd(), record.id, { budgetMs: 300 });
...
test("a job that genuinely does not exist still throws", async () => {
  ...
  await assert.rejects(
    () => followJob(process.cwd(), "task-nope", { budgetMs: 2000 }),
    /No job found/,
    "tolerating torn reads must not swallow a real unknown-job error"
  );
});

test("the follower writes nothing to stderr, even when the log has content", async () => {
  freshRoot();
  const root = process.env.CODEX_COMPANION_STATE_ROOT;
  const state = await import(`${stateUrl}?f=${seq}`);
  const { followJob } = await import(`${followUrl}?f=${seq}`);
  fs.mkdirSync(root, { recursive: true });
  const logFile = path.join(root, "quiet.log");
  fs.writeFileSync(logFile, "[t] Turn started.\n[t] Assistant message\nthinking out loud\n", "utf8");
  const record = jobRecord({ id: "task-quiet", logFile });
  state.writeJobFile(process.cwd(), record.id, record);
  state.upsertJob(process.cwd(), record);
  setTimeout(() => state.upsertJob(process.cwd(), { id: record.id, status: "completed" }), 200);

  const chunks = [];
  const originalWrite = process.stderr.write;
  process.stderr.write = (chunk) => { chunks.push(String(chunk)); return true; };
  try {
    const snapshot = await followJob(process.cwd(), record.id, { budgetMs: 3000 });
    assert.equal(snapshot.job.status, "completed");
  } finally {
...
test("the handback names the job, the workspace, and how to get the result", async () => {
  ...
  assert.match(text, /--wait/, "must name the result --wait pickup command");
